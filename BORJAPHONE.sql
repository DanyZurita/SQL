...
/* 3. */
    ...
    ALTER TABLE SERVICES ADD CONSTRAINT services_fk FOREIGN KEY (CODE_SERVICE) REFERENCES SERVICE_TYPES(CODE) ON UPDATE CASCADE ON DELETE CASCADE;

    /* N. */

    ALTER TABLE INVOICE_LINES ADD CONSTRAINT invoices_lines_fk_2 FOREIGN KEY (CODE_SERVICE) REFERENCES SERVICES(CODE) ON UPDATE CASCADE ON DELETE CASCADE; 

    /* O. */

    SHOW CREATE TABLE INVOICE_LINES;
